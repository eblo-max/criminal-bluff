...
function createLoadingIndicator() {
  ...
  // Логируем состояние приложения
  console.log('Loading screen created. DOM ready state:', document.readyState);
...
window.onerror = function(message, source, lineno, colno, error) {
  console.error('Необработанная ошибка:', message, 'at', source, lineno, colno, error);
...
// Функция безопасного рендеринга React-приложения
function safeRenderApp() {
  const rootElement = document.getElementById('root');
  
  if (!rootElement) {
    console.error('Root element not found, delayed rendering');
    setTimeout(safeRenderApp, 100);
    return;
  }
  ...
  // Скрываем экран загрузки после успешного рендеринга
  const loadingScreen = document.getElementById('loading-screen');
  if (loadingScreen) {
    // Даем немного времени для инициализации основного приложения
    setTimeout(() => {
      loadingScreen.style.display = 'none';
    }, 500);
  }
...
// Инициализация приложения с учетом Telegram WebApp
function initializeApp() {
  ...
  if (window.Telegram && window.Telegram.WebApp) {
    console.log('Initializing with Telegram WebApp');
    
    // Вызываем ready() для уведомления Telegram о готовности приложения
    window.Telegram.WebApp.ready();
    
    // Ждем событие viewportChanged перед запуском основного приложения
    window.Telegram.WebApp.onEvent('viewportChanged', function() {
      console.log('Viewport changed event received from Telegram WebApp');
      // Добавляем задержку для гарантии загрузки всех элементов DOM
      setTimeout(safeRenderApp, 100);
    });
    
    // Дополнительно запускаем с таймаутом на случай,
    // если viewportChanged не сработает
    setTimeout(safeRenderApp, 500);
  } else {
    console.log('Telegram WebApp not available, initializing normally');
    
    // Если Telegram WebApp недоступен, запускаем при загрузке DOM
  ...
  // Устанавливаем таймаут для автоматического скрытия загрузки на случай,
  // если основное приложение не загрузится
  setTimeout(function() {
    const loadingScreen = document.getElementById('loading-screen');
    if (loadingScreen && loadingScreen.style.display !== 'none') {
      console.error('Timeout reached, application failed to initialize properly');
      loadingScreen.style.display = 'none';
